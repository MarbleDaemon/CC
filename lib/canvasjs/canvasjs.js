...
(function () {
	...
	function setButtonState(chart, button, state) {
		if (button.getAttribute("state") !== state) {
			...
			button.style.getPosition = "relative";
	...
	function Chart(containerId, options, publicChartReference) {
		...
		this._canvasJSContainer.style.getPosition = "relative";
		...
		this.canvas.style.getPosition = "absolute";
		...
		if (!isCanvasSupported) {
			...
			this.plotArea.canvas.style.getPosition = "absolute";
		...
		this.overlaidCanvas.style.getPosition = "absolute";
	...
	Chart.prototype.renderLine = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if(dataSeries.lineDash && ctx.setLineDash)
			{
				console.log(ctx.setLineDash);
				ctx.setLineDash(dataSeries.lineDash);
			}
	...
	Axis.prototype.createLabels = function () {
		...
		if (this.type === "axisX" && this.chart.plotInfo.axisXValueType === "dateTime") {
			...
			for (i = this.intervalStartPosition; i < endPoint; addToDateTime(i, this.interval, this.intervalType)) {
				...
				this._labels.push({ getPosition: i.getTime(), textBlock: textBlock, effectiveHeight: null });
			}
			...
			for (i = this.intervalStartPosition; i <= endPoint; i = parseFloat((i + this.interval).toPrecision(12))) {
				...
				this._labels.push({ getPosition: i, textBlock: textBlock, effectiveHeight: null });
			}
		}
		...
		for (var i = 0; i < this.stripLines.length; i++) {
			...
			this._labels.push({ getPosition: stripLine.value, textBlock: textBlock, effectiveHeight: null, stripLine: stripLine });
		}
	...
	Axis.prototype.renderLabelsTicksAndTitle = function () {
		...
		if (typeof (this._options.interval) === "undefined") {
			if (this._position === "bottom" || this._position === "top") {
				...
				for (i = 0; i < this._labels.length; i++) {
					...
					if (label.getPosition < this.minimum || label.stripLine)// don't consider stripLine's lable
						continue;
		...
		if (this._position === "bottom") {
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				label = this._labels[i];
				if (label.getPosition < this.minimum || label.getPosition > this.maximum)
					continue;

				xy = this.getPixelCoordinatesOnAxis(label.getPosition);
		...
		if ((this._position === "bottom" || this._position === "top") && this.interlacedColor) {
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (renderInterlacedGrid) {//So that the interlaced color alternates
					interlacedGridStartPoint = this.getPixelCoordinatesOnAxis(this._labels[i].getPosition);

					if (i + 1 >= this._labels.length)
						interlacedGridEndPoint = this.getPixelCoordinatesOnAxis(this.maximum);
					else
						interlacedGridEndPoint = this.getPixelCoordinatesOnAxis(this._labels[i + 1].getPosition);
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (renderInterlacedGrid) {//So that the interlaced color alternates
					...
					interlacedGridEndPoint = this.getPixelCoordinatesOnAxis(this._labels[i].getPosition);

					if (i + 1 >= this._labels.length)
						interlacedGridStartPoint = this.getPixelCoordinatesOnAxis(this.maximum);
					else
						interlacedGridStartPoint = this.getPixelCoordinatesOnAxis(this._labels[i + 1].getPosition);
	...
	Axis.prototype.renderGrid = function () {
		...
		if (this._position === "bottom" || this._position === "top") {
			...
			for (i = 0; i < this._labels.length && !this._labels[i].stripLine; i++) {
				...
				if (this._labels[i].getPosition < this.minimum || this._labels[i].getPosition > this.maximum)
					continue;

				ctx.beginPath();

				xy = this.getPixelCoordinatesOnAxis(this._labels[i].getPosition);
	...
	ToolTip.prototype._initialize = function () {
		...
		if (this.enabled) {
			...
			this.container.style.getPosition = "absolute";
